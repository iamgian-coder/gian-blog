  ...
  <div class="search-container">
    <div class="searchTip">
      ...
      <div class="count">
        共搜索到
        <span>
          <strong>{{ response.totalDocs }}</strong>
        </span>
        篇
      </div>
    </div>
    <article
      ...
      :key="index"
      class="article"
      :class="`flag${index}search`"
    >
      <after-bgc :index="index" tail="search" />
...
const gqlArticlesBySearching = `query articlesBySearching($keyword: String!, $page: Int!, $limit: Int!) {
    getArticlesBySearching(keyword: $keyword,page: $page, limit: $limit, where: {}, sort: {}) {
      docs {
        _id
        title
        markdown
...
export default {
  ...
  asyncData(ctx) {
    ...
    return $axios
      ...
      .then(({ data }) => {
        return {
          keyword,
          response: data.getArticlesBySearching
        };
      })
      .catch((err) => {
  ...
  methods: {
    async infiniteHandler($state) {
      ...
      const { $axios, $apipathMain, keyword } = this;
      if (hasNextPage) {
        ...
        const response = data.getArticlesBySearching;
...
.search-container {
  > .searchTip {
    ...
    > .word {
      padding: $gapSize * 2 $gapSize * 2 0 $gapSize * 2;
    }
    > .count {
      padding-top: $gapSize;
