<template>
  <div class="tag-container">
    ...
    <article
      ...
      :key="index"
      class="article"
      :class="`flag${index}tag`"
    >
      <after-bgc :index="index" tail="tag" />
...
const gqlArticlesByPage = `query articlesByTag($page: Int!, $limit: Int!, $where: QueryWhereInput!) {
  getArticlesByPage(page: $page, limit: $limit, where: $where, sort: {}) {
    docs {
      _id
      title
      markdown
...
export default {
  ...
  asyncData(ctx) {
    ...
    return $axios
      ...
      .then(({ data }) => {
        return {
          where,
          response: data.getArticlesByPage
        };
      })
      .catch((err) => {
  ...
  methods: {
    async infiniteHandler($state) {
      ...
      const { $axios, $apipathMain, where } = this;
      if (hasNextPage) {
